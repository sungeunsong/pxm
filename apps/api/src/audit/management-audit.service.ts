...
export type ManagementAuditEvent = {
  event_id?: string;
  action: string;
  resource_type: 'workflow' | 'task' | 'group' | 'user' | 'service_account' | 'api_key' | 'credential' | 'security_policy' | 'runtime_integrity' | 'webhook_endpoint' | 'webhook_delivery' | 'runtime_operation' | 'approval_notification';
  resource_id: string;
