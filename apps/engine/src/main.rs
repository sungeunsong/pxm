...
async fn main() -> Result<()> {
    ...
    let v2_context = if db_type == "mongodb" {
        let mongo_url = std::env::var("MONGODB_URL").unwrap_or_else(|_| "mongodb://127.0.0.1:27017".to_string());
        ...
        let is_replica_set = match admin_db.run_command(mongodb::bson::doc! { "isMaster": 1 }, None).await {
            Ok(res) => res.contains_key("setName"),
            Err(_) => false,
        };
        println!("[engine] MongoDB replica set status: is_replica_set={is_replica_set}");
        
        let adapter = v2::infrastructure::mongo_adapter::MongoAdapter::new(client, &db_name, is_replica_set);
