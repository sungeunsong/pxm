...
seed는 그룹, 사용자 4명, 자격증명 2개, 직원 샘플 3건, 배포된 워크플로우 3개와 워크플로우별 입력 프리셋 2개를 준비한다. 같은 데이터는 다시 만들거나 버전을 올리지 않는다. 데모 워크플로우를 편집했다면 기준 그래프로 복구하며 새 버전으로 배포한다. 자유롭게 설계하는 연습은 워크플로우를 복제해서 한다.
