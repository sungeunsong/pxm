...
화면에 `배포 vnull`이 뜨던 원인입니다. PXM-33에서 UI의 null 방어를 유지하면서,
배포 저장 시 불변 버전 존재 여부와 배포 메타데이터를 검증하도록 수정했습니다.
기존 데이터 보정 절차와 집계 기준은 [workflow-publish-metadata-repair.md](workflow-publish-metadata-repair.md)를 참고합니다.
