...
하나라도 실패하면 명령이 0이 아닌 종료 코드를 반환하므로 CI 또는 배포 작업은 `pnpm gate:beta` 성공을 필수 조건으로 사용한다. 실행기는 실패 지점을 다음 종료 코드로 구분한다.

| 종료 코드 | 의미 |
|---|---|
| `1` | Playwright 테스트 또는 데모 시나리오가 실제로 실패함 |
| `2` | Docker, DB, 포트, 서버 기동 등 테스트 인프라 준비 실패 |
| `3` | 테스트가 한 건도 실행되지 않았거나 결과 리포트를 읽지 못함 |
