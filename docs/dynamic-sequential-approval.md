...
- 단계는 1부터 시작해 빈 번호 없이 연속이어야 하며, 각 단계에는 비어 있지 않은
  `assignee`가 정확히 하나 있어야 한다.
...
| 동작             | 현재 단계  | 다음 단계             | ApprovalRequest | Engine         |
| ---------------- | ---------- | --------------------- | --------------- | -------------- |
| 중간 승인        | `APPROVED` | `OPEN`, Task 1개 생성 | `IN_PROGRESS`   | 계속 `WAITING` |
| 최종 승인        | `APPROVED` | 없음                  | `APPROVED`      | `RESUME` 1회   |
| 어느 단계든 반려 | `REJECTED` | 이후 단계는 `LOCKED`  | `REJECTED`      | `RESUME` 1회   |

Task 완료, 단계 전이, 다음 Task 생성 또는 최종 `RESUME` 등록은 하나의 DB
트랜잭션에서 처리한다. Task는 `approval_step_id`별로 하나만 존재할 수 있어 중복 요청이
다음 단계를 두 번 여는 것을 막는다.

## 이번 차수에서 제외된 범위

한 단계에 여러 승인자를 두는 ALL/ANY 정책은 PXM-10, 외부 요청 중복 방지와 실행 중
결재라인 변경/개정 정책은 PXM-11, 반려 전용 워크플로우 분기는 PXM-12에서 다룬다.
